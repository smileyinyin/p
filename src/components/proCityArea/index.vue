<template>
    <Row type="flex" :gutter="10">
        <i-col :span="span">
            <Select v-model="proCode" placeholder="省" :label-in-value="false" clearable @on-change="changeProvince">
                <Option v-for="item in proList" :key="item.id" :value="item.identity">{{ item.name }}</Option>
            </Select>
        </i-col>
        <i-col :span="span" v-if="!onlyProvince">
            <Select v-model="cityCode" placeholder="市" :label-in-value="false" clearable @on-change="changeCity">
                <Option v-for="item in cityList" :key="item.id" :value="item.identity">{{ item.name }}</Option>
            </Select>
        </i-col>
        <i-col :span="span" v-if="!onlyProvince && !noArea">
            <Select v-model="areaCode" placeholder="区" :label-in-value="false" clearable @on-change="changeArea">
