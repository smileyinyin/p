  ...
  <div>
    ...
    <Card>
      ...
      <store-base-info
        ...
        :baseInfo="baseInfo"
      ></store-base-info>
      <!-- <divide-line placement="bottom" class="mb20">绑定信息</divide-line>
      <store-account-info
        ref="accountInfo"
...
import storeAccountInfo from './components/storeAccountInfo'
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      //   this.$refs['accountInfo'].submit()
      // ]).then(([baseInfo, accountInfo]) => {
      //   // 有id是更新
      //   if (this.id) {
      //     Promise.all([
      //       updateStore(baseInfo),
      //       merchantSignUp(accountInfo)
      //     ]).then(res => {
      //       this.$router.push({ name: 'storeList' })
      //     })
      //   } else {
  ...
  components: { breadCrumb, divideLine, storeBaseInfo, storeAccountInfo }
}
</script>
