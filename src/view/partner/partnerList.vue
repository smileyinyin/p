      ...
      <Row :gutter="10" class="mb20">
        <i-col :span="24" style="display:flex">
          ...
          <Row type="flex" justify="end" style="flex:1">
            <Input class="w200 mr20" v-model="searchForm.mobile" placeholder="合伙人账号（手机号码）"/>
          ...
          render: (h, params) => {
            ...
            switch (status) {
              case 1:
                act = '审核'
                type = 'warning'
                onclick = () => this.$router.push({ name: 'partnerCheck', query: { id: params.row.id } })
                break
              case 2:
                act = '禁用'
                type = 'error'
            ...
            return h('div', [
              h('Button', {
                ...
                on: {
                  click: status === 2
                    ? () => this.$router.push({ name: 'partnerCheck', query: { id: params.row.id } })
                    : () => this.$router.push({ name: 'partnerAdd', query: { id: params.row.id } })
                }
              }, status === 2 ? '查看' : '修改'),
              act && h('Button', {
